//
//  Color.swift
//  CooksCorner
//
//  Created by Камаль Атавалиев on 12.03.2024.
//

import UIKit

extension UIColor {
    static let mainOrangeColor = #colorLiteral(red: 0.9804651141, green: 0.6187419295, blue: 0.1972349584, alpha: 1)
    static let lightOrangeColor = #colorLiteral(red: 1, green: 0.9674473405, blue: 0.8682621121, alpha: 1)
    static let textFieldBackGround = #colorLiteral(red: 0.9294117093, green: 0.9294117093, blue: 0.9294117093, alpha: 1)
    #warning("Сделать литерал норм")
    static let blackoutColor = UIColor(red: 0/255, green: 0/255, blue: 0/255, alpha: 0.4)
    
}
